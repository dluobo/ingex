/*
 * $Id: Threads.h,v 1.1 2009/02/24 08:21:16 stuart_hc Exp $
 *
...
#ifndef __INGEX_THREADS_H__
#define __INGEX_THREADS_H__


#include <pthread.h>


#define LOCK_SECTION(m)             ingex::MutexLocker __lock(m);
#define LOCK_SECTION_2(m)           ingex::MutexLocker __lock2(m);
#define LOCK_SECTION_3(m)           ingex::MutexLocker __lock3(m);

#define TRY_LOCK_SECTION(m)         ingex::TryMutexLocker __tryLock(m);
#define HAVE_SECTION_LOCK()         __tryLock.haveLock()

#define READ_LOCK_SECTION(rwl)      ingex::ReadLocker __readLock(rwl);
#define WRITE_LOCK_SECTION(rwl)     ingex::WriteLocker __writeLock(rwl);

#define GUARD_THREAD_START(hs)      ingex::HasStoppedGuard __guard(hs)
