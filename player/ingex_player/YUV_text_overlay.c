...
static YUV_error allocate_info(p_info_rec* p_info)
{
    info_rec*	info;
...
static YUV_error find_font(info_rec* info, char* family, char* path)
{
    FcPattern*	pattern;
...
static YUV_error set_font(info_rec* info, char* family, int size,
...
YUV_error text_to_overlay(p_info_rec* p_info, overlay* ovly, char* text,
                          ...
                          const int aspect_ratio_num, const int aspect_ratio_den)
{
    ...
    {
        count = text_to_overlay(info, &line_ovly[no_lines], sub_str, max_width, 0,
                       ...
                       const int aspect_ratio_num, const int aspect_ratio_den)
{
    ...
    for (n = 0; n < 4; n++)
    {
        result = text_to_overlay(info, &txt_ovly[n], title[n],
                                 (max_width / 2) - 3, 0, 0, 0, 0, 0, 0, font, size,
...
YUV_error init_timecode(p_info_rec* p_info, timecode_data* tc_data,
                        char* font, const int size,
                        const int aspect_ratio_num, const int aspect_ratio_den)
{
    info_rec*		info;
