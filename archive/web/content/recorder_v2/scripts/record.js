...
function set_digibeta_dropout_visibility(visible)
{
    ...
    {
        dropoutCountHeadE.setAttribute("style", "");
        dropoutCountE.setAttribute("style", "");
        set_tag_state(dropoutCountHeadE, true);
    }
    else if (!visible && visibleState)
    {
        dropoutCountHeadE.setAttribute("style", "display: none;");
        dropoutCountE.setAttribute("style", "display: none;");
...
function set_session_status(status)
{
    ...
    else
    {
        ...
        update_tag_value("vtr-state", status.sessionStatus.vtrState, escape(status.sessionStatus.vtrState));
        update_tag_value("vtr-error-count", status.sessionStatus.vtrErrorCount, status.sessionStatus.vtrErrorCount);
        if (status.sessionStatus.digiBetaDropoutEnabled)
        {
            set_digibeta_dropout_visibility(true);
            update_tag_value("digibeta-dropout-count", status.sessionStatus.digiBetaDropoutCount, status.sessionStatus.digiBetaDropoutCount);
        }
        else
        {
