...
MXFWriter::MXFWriter(RecorderInputConfig* inputConfig, SourceSession* sourceSession, 
  ...
  _failuresFilePath(failuresFilePath), _filenamePrefix(filenamePrefix)
{
        ...
        iterTrackConfigs++)
    {
        ...
        if ((inputMask & (0x01 << (trackConfig->index - 1))) && trackConfig->isConnectedToSource())
        {
            ...
            OutputPackage* outputPackage = getOutputPackage(sourceSession, trackConfig->sourceConfig->name);
            if (outputPackage == 0)
            {
                ...
                outputPackage->sourcePackage = sourceSession->getSourcePackage(trackConfig->sourceConfig->name);
                if (outputPackage->sourcePackage == 0)
                {
                    PA_LOGTHROW(MXFWriterException, ("Failed to get source package associated with recorder input track"));
                }
...
OutputPackage* MXFWriter::getOutputPackage(SourceSession* sourceSession, string sourceName)
{
    vector<OutputPackage*>::const_iterator iter;
    for (iter = _outputPackages.begin(); iter != _outputPackages.end(); iter++)
    {
        if ((*iter)->sourcePackage == sourceSession->getSourcePackage(sourceName))
        {
            return *iter;
        }
    }
