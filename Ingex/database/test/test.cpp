/*
 * $Id: test.cpp,v 1.1 2006/12/19 16:44:53 john_f Exp $
 *
...
static void test_source_session()
{
    ...
    auto_ptr<SourceConfig> sourceConfigInDB;
    auto_ptr<Recorder> recorderInDB;;
    auto_ptr<SourceSession> sourceSessionInDB;
    ...
    try
    {
        ...
        // create
        sourceSessionInDB = auto_ptr<SourceSession>(SourceSession::create(recorderInDB.get(), 
            generateDateNow()));


        // clean up
        if (sourceSessionInDB.get() != 0)
        {
            sourceSessionInDB->deleteFromDatabase();
        }
        if (recorderInDB.get() != 0 && recorderInDB->isPersistent())
        {
            database->deleteRecorder(recorderInDB.get());
...
int main(int argc, const char* argv[])
{
    ...
    string username = "ingex";
    string password = "ingex";
