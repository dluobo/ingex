/*
 * $Id: RouterConfig.cpp,v 1.1 2007/01/30 12:46:20 john_f Exp $
 *
...
bool RouterConfig::getSourcePackage(uint32_t index, SourceSession* session, SourcePackage** package, 
    uint32_t* sourceTrackID)
{
    ...
    if ((sourcePackage = session->getSourcePackage(sourceConfig->name)) == 0)
    {
        // no source package in session associated with source config
        return false;
    }
