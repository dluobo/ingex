/*
 * $Id: CreateAAF.cpp,v 1.1 2007/02/08 10:42:59 philipn Exp $
 *
    ...
    try
    {
        ...
        for (iter = materialPackages.begin(); iter != materialPackages.end(); iter++)
        {
            ...
            for (iter2 = materialPackage->tracks.begin(); iter2 != materialPackage->tracks.end(); iter2++)
            {
                ...
                if (getSource(track, packages, &sourcePackage, &sourceTrack))
                {
                    ...
                    for (iter3 = mcClipDef->trackDefs.begin(); !donePackage && iter3 != mcClipDef->trackDefs.end(); iter3++)
                    {
                        ...
                        for (iter4 = trackDef->selectorDefs.begin(); !donePackage && iter4 != trackDef->selectorDefs.end(); iter4++)
                        {
                            ...
                            if (SourceSession::isInstancePackage(selectorDef->sourceConfig, sourcePackage) && sourceTrack->id == selectorDef->sourceTrackID)
                            {
...
bool AAFFile::getSource(Track* track, PackageSet& packages, SourcePackage** sourcePackage, Track** sourceTrack)
{
    if (track->sourceClip->sourcePackageUID == g_nullUMID)
    {
        return false;
    ...
    if (result != packages.end())
    {
        ...
        // go further down the reference chain to find the source
        return getSource(trk, packages, sourcePackage, sourceTrack);
    }
...
// TODO: handle case where master mobs have different lengths
// look at the file source package source clip startTime to work out
// where to put the fillers
