...
//                    fprintf(stderr, "Label %s\n", optarg);
    ...
    // buffer for DV data
    uint8_t compressedBuffer[compressed_size];
    // buffer for decompressed video
    AVFrame* decFrame = avcodec_alloc_frame();
    if(!decFrame) {
    ...
    if (!alloc_YUV_frame(&out_frame, width, height, Y42B))
    {
    ...
    for (n = 0; n < 4; n++)
    {
        ...
        {
            if (lseek(input[n].fd,
                      input[n].skip * compressed_size, SEEK_SET) < 0)
                perror("lseek");
        }
    }
...
*/
        count = ml_text_to_ovly(&at_info, &label[0].ovly, label_string,
                                (width / 2) - 40, "Helvetica", 32,
                                guess_par(width, height, 16, 9));
        if (count < 0)
            exit(1);
        add_overlay(&label[0].ovly, &out_frame, label[0].x, label[0].y,
                    235, 0, 0, 40);
    }
    ...
    while (1)
    {
        ...
        for (n = 0; n < 4; n++)
        {
            ...
            {
                // Read and decode an input frame
                if (read_all(input[n].fd, compressedBuffer, compressed_size))
                {
                    OK = 1;     // still got an input
                    int finished;
                    avcodec_decode_video(dec, decFrame, &finished,
                                         compressedBuffer, compressed_size);
                    if(!finished)
                    {
                        fprintf(stderr, "error decoding video\n");
                        exit(1);
                    }
                    // Map decFrame to in_frame
                    if (YUV_frame_from_AVFrame(&in_frame, dec, decFrame) < 0)
                        exit(1);
                }
                else
                {
