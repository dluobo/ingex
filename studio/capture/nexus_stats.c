...
extern int main(int argc, char *argv[])
{
    ...
    NexusTimecode   tc_type = NexusTC_DEFAULT;
    ...
    if (verbose) {
        ...
        printf("  default_tc_type=%s master_tc_channel=%d\n",
                nexus_timecode_type_name(pctl->default_tc_type),
    ...
    while (1)
    {
        ...
        for (i = 0; i < pctl->channels; i++)
        {
            ...
            tc[i] = nexus_tc(pctl, ring, i, lastframe, tc_type);
            signal_ok[i] = nexus_signal_ok(pctl, ring, i, lastframe);
            num_aud_samp[i] = nexus_num_aud_samp(pctl, ring, i, lastframe);
