// GalleryLoggerDoc.cpp : implementation of the CGalleryLoggerDoc class
//
...
: mSeriesIndex(-1), mProgrammeIndex(-1), mItemIndex(-1), mCurrentSequenceIndex(-1), mIsEmpty(true), mBackupIndex(0)
{
...
void CGalleryLoggerDoc::Import(const char * filename) 
{
	...
	int count = swdfile.OrderedSequence.size();
	int i;
	for(i=0; i<count; ++i)
	{
...
void CGalleryLoggerDoc::DeleteSequence()
{
	// point iterator at current sequence
	std::vector<SequencePtr>::iterator it = mRecordingOrder.begin();
	it += mCurrentSequenceIndex;
	mRecordingOrder.erase(it);
