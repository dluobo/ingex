...
: wxDialog(parent, wxID_ANY, (const wxString &) wxT("Ingex Control Help"), wxDefaultPosition, wxSize(600, 400), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) //cygwin needs the cast
{
	...
	message = wxT("Each recording must have an associated project name, which is shown in the window header as well as in the #Record# tab.  The list of project names and the one which is selected are remembered between sessions.\n\nThe first time the controller is run, it will not be able to find any saved project names, so will immediately produce the Set Project Names dialogue.  This will allow you to add a project name or accept the default suggestion.\n\nYou may access the dialogue again from the #Misc# menu, to add, change, select and delete project names.");
	...
	message = wxT("Recordings cannot be made unless all enabled sources (apart from those of router recorders) have a corresponding tape ID.  #Set tape IDs# will be orange if a missing tape ID is preventing you from recording.  Pressing this button (whether orange or not) brings up a dialogue for manipulating tape IDs, which are remembered between sessions.  It is designed to allow you to enter and update the information for several tapes quickly.  Press the #Help# on the dialogue for details about how to do this.");
	...
	message = wxT("Below the transport controls is the #Record# tab, which can be clicked to reveal the #Project# name (as set from the #Misc# menu), the #Description# field and the record source tree.  The space given to this tab can be varied by dragging the dotted splitter line below it.\n\nThe contents of the #Description# field is read when #Stop# is pressed, and is associated with the recording just made.  Note that while the box has focus (i.e. it contains the cursor and you can type into it), short cut keys which would interfere with the control (such as #J#, #K# and #L#) will type into the box rather than perform their normal short-cut function.\n\nThe source tree, below #Description#, can be expanded to four levels by clicking the small arrows.  These levels show each individual media source (audio [yellow], video [orange] or router [green]), grouped by package, in turn grouped by recorder.  At the package level, the tape ID is shown in brackets after the package name.  (To configure this, press #Set Tape IDs#.)  The source tree will also show display status messages at the recorder level and the source level.\n\nWhile not recording, the symbol next to each entry in the tree can be clicked to toggle record enable for it and all child entries.  The symbols for the packages and above will reflect the individual sources, with a partially enabled indication if appropriate.  While recording, the symbols change to show recording or partially recording.\n\nIf there is a problem or an inability to communicate, an alert symbol (exclamation mark) or query symbol (question mark) is shown.  This takes priority over the other indications, so parent symbols will reflect the status.  (The health indicator next to the timecode display will also show that a problem has occurred.)  Each recorder is interrogated every ") + wxString::Format(wxT("%d second%s"), WATCHDOG_TIMER_LONG_INTERVAL/1000, WATCHDOG_TIMER_LONG_INTERVAL/1000 == 1 ? wxT("") : wxT("s")) + wxT(" or so (more frequently during abnormal situations) and the symbols are updated accordingly.");
	StyleAndWrite(sources, message);
