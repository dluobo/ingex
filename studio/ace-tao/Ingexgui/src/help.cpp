/***************************************************************************
 *   $Id: help.cpp,v 1.27 2012/02/10 15:12:55 john_f Exp $                *
 *                                                                         *
...
: wxDialog(parent, wxID_ANY, (const wxString &) wxT("Ingex Control Help"), wxDefaultPosition, wxSize(600, 400), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) //cygwin needs the cast
{
    ...
    message = wxT("When connected to at least one recorder, a dialogue can be accessed from the #Misc# menu to set pre-roll and post-roll.  These settings are remembered between sessions.\n\nThe #Pre-roll# slider allows you to determine how many frames into the past are added to the beginning of a recording.  This can be useful to catch action but also reduces the possibility of recorders being asked to record a frame which is not yet available (causing the recording to fail).  The maximum value of pre-roll depends on the capabilities of the recorders.\n\nThe #Post-roll# slider allows you to determine how many additional frames are recorded after the stop button is pressed.  Recorders are likely to allow an infinite amount of post-roll, in which case it is limited to ") + wxString::Format(wxT("%d frame%s."), DEFAULT_MAX_POSTROLL, DEFAULT_MAX_POSTROLL == 1 ? wxEmptyString : wxT("s")) + wxT("  During post-roll the record button shows the \"running down\" state");
