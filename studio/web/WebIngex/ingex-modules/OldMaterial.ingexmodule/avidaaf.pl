...
#                $includeDirectorsCut && $dcDb ? "--mc-cuts \"$dcDb\"" : "", # director's cut database
                ...
                "--dbhost $ingexConfig{'db_host'}", # database host name
                "-d $ingexConfig{'db_name'}", # database name
                "-u $ingexConfig{'db_user'}", # database user
                $projNameStr ?
                        "-n \'$projNameStr\'" : "", # project name
                "--dbpassword $dbPassword", # database password
                $includeDirectorsCut ? "--mc-cuts-db" : "", # include director's cuts from main database
                ">$resultsFilename"
        );
...
# Director's cut source is now held in database
