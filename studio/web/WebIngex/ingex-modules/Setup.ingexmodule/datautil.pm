...
sub create_source_config
{
    ...
    foreach (1..$numVideo)
    {
        _add_source_config_track($scf, $db::dataDef{"Picture"});
    }
    foreach (1..$numAudio)
    {
        _add_source_config_track($scf, $db::dataDef{"Sound"});
    }
...
sub _add_source_config_track
{
    ...
    my $trackName = ($datadefid == $db::dataDef{"Picture"}) ? "V".$trackNumber : "A".$trackNumber;
