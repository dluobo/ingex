...
{
    db::delete_series($dbh, $Id) or
        return_error_page("failed to delete series with id=$Id from database: $prodautodb::errstr");    
...
my $page = get_content($series) or
    return_error_page("failed to fill in content for delete series page");
...
sub get_content
{
    ...
    push(@pageContent, h1('Delete series name'),
        start_form({-id=>"ingexForm", -action=>"javascript:sendForm('ingexForm','deleteseries')"}),
        hidden(-name=>"id", -default=>$series->{'ID'}),
        p("Please confirm"),
