...
# Copy the CGI files into these directories - EXCLUIDE CONFIG FILES so as not to overwrite the user's settings
rsync -arC --exclude=*.conf ingex-modules /srv/www/cgi-bin/
rsync -arC --exclude=*.conf ingex-config /srv/www/cgi-bin/
