...
extern int main(int argc, char *argv[])
{
	...
	mjpeg_compress_threaded_t *mc = NULL;
	mjpeg_compress_init_threaded(resId, width, height, &mc);
	...
	while (1) {
		...
		do {
			...
			//times(&buf1);
			if ((outsize = mjpeg_compress_frame_yuv_threaded(mc, y, u, v, width, width / 2, width / 2, &outputframe)) == 0) {
	...
	mjpeg_compress_free_threaded(mc);
