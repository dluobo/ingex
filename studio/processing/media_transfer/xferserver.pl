...
#/***************************************************************************
# * $Id: xferserver.pl,v 1.24 2011/11/30 12:13:05 john_f Exp $             *
# *                                                                         *
...
use IO::File;
use Getopt::Std;
our $VERSION = '$Revision: 1.24 $'; #used by Getopt in the case of --version or --help
...
sub HELP_MESSAGE { #given this name, getopts will call this subroutine if given "--help" as a command-line option.
 ...
 -n  do not copy or delete (merely update paths file with event information)
...
HELP_MESSAGE() unless getopts('a:cde:f:g:mnprstv', \%opts)
...
sub addPair {
 ...
 if (!exists $transfers->{staleFrom}) { #not pending rescan
    print CHILD "\n" unless $opts{n}; #will kick the child into action if it's idle
 }
