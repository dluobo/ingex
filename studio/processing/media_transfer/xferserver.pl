...
# This script controls copying of material from the local host to a remote file server, under the instruction of clients, applying switchable traffic control to prevent saturation of the local disks.
...
# Additional parameters indicate the end of the event, or a recorder being started.  This will result in the event's identifier being removed from the event list (if present), and if the list is then empty, traffic control being switched off (reverting to copying at FAST_LIMIT, which is unlimited if set to zero).
...
# For each priority, the files to copy are sorted by decreasing age as indicated by ctime, making sure that all files with the latest detected ctime have been accounted for.  Before copying a file, the amount of free space is checked to make sure it is likely to fit, and further copying from this source directory is abandoned (until retry) if it doesn't: this avoids wasting time repeatedly copying large files that won't succeed.  When copying of the last file with a particular ctime in a particular source directory is completed, this ctime is written to the configuration file, and any files older than this are subsequently ignored.  This allows files to be removed from the destination without them being copied again if they remain on the source.  (ctime is used rather than mtime, because it indicates the time the source file was made visible by moving it into the source directory, therefore making it impossible for older files, which would never be copied, to appear subsequently.)
...
# If option -e is supplied, the path specified with it forms an additional destination for all files at priority '1'.  This is intended for use with a portable drive onto which to copy offline rushes.
...
die "Usage: xferserver.pl [-c] [-d] [-e <path>] [-p] [-r] [-s] [-v]
 ...
 -v        verbose: print out every source file name with reasons for not copying\n"
unless getopts('cde:prsv', \%opts) && (!defined $opts{e} || ($opts{e} ne '' && $opts{e} !~ /^-/));
...
sub Scan {
 my ($handle, $srcDir, $destRoot, $ctimeCleared, $share, $extraDestRoot, $extraCtimeCleared) = @_;
 ...
 while (@files && ($ok || $extraOk)) {
	...
	my @checkDest = CheckDest($srcDir, $destDir, $srcName, $size, $altName, $share); #even if this dir has been abandoned, still check for a copy to see if we can delete the source file
	$ok &= $checkDest[0];
	...
	if (defined $extraDestDir) {
		...
		@checkDest = CheckDest($srcDir, $extraDestDir, $srcName, $size, $altName, $share, \$extraCopied); #even if this dir has been abandoned, still check for a copy to see if we can delete the source file
		$extraOk &= $checkDest[0];
...
sub CheckDest
{
 my ($srcDir, $destDir, $name, $size, $altName, $share) = @_;
...
sub childLoop {
 ...
 while (1) {
	...
	foreach my $priority (@priorities) { #NB share locked at this point
		...
		foreach (sort keys %pairs) {
			...
			do { #loop to make sure we get all the files with the latest ctime we find
				...
				@scan = Scan(*SRC, $_, $pairs{$_}{dest}, $pairs{$_}{ctimeCleared}, $share, $extra ? $transfers->{extraDest} : undef, $pairs{$_}{extraCtimeCleared}); #enable extra destination if at priority 1 and it is supplied
		...
		foreach my $essenceFile (sort { $a->{ctime} <=> $b->{ctime} } @essenceFiles) { #NB share locked at this point
			...
			$incomingDirs{$incomingPath} = 1; #so they can be deleted on completion
			# check free space
	...
	foreach (keys %incomingDirs) {
		rmdir; #do this in all cases because config might have changed
	}
 }
}
