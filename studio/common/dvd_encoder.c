/*
 * $Id: dvd_encoder.c,v 1.1 2007/09/11 14:08:36 stuart_hc Exp $
 *
...
static int initialise_tc_mask (internal_dvd_encoder_t *dvd)
{
	dvd->tc_width = 199;
	dvd->tc_height = 24;
	dvd->tc_xoffset = 460;
...
static void setup_tc_mask(internal_dvd_encoder_t *dvd, int frame_number)
{
...
AVStream *add_video_stream(AVFormatContext *oc, int codec_id, int bit_rate)
{
    ...
    c->width = 720;  
    c->height = 576;
    c->sample_aspect_ratio = av_d2q(16.0/9*576/720, 255);
...
static int open_video(internal_dvd_encoder_t *dvd)
{
...
static void close_video(internal_dvd_encoder_t *dvd)
{
	AVStream *st = dvd->video_st;
...
static int open_audio(internal_dvd_encoder_t *dvd)
{
...
void close_audio(internal_dvd_encoder_t *dvd)
{
	AVStream *st = dvd->audio_st;
    avcodec_close(st->codec);
...
static void cleanup (internal_dvd_encoder_t *dvd)
{
	if (dvd)
	{
		if (dvd->video_st)
...
{
	internal_dvd_encoder_t *dvd;
	AVOutputFormat *fmt;

	/* register all ffmpeg codecs and formats */
	av_register_all();

	/* Check if dvd format exists */
	fmt = guess_format ("dvd", NULL, NULL);
	if (!fmt)
	{
		fprintf (stderr, "dvd format not registered\n");
		return NULL;
	}

	dvd = av_mallocz (sizeof(internal_dvd_encoder_t));
	if (!dvd)
	{
	...
	dvd->oc = av_alloc_format_context();
	if (!dvd->oc)
	{
		cleanup(dvd);
	...
	dvd->oc->oformat = fmt;
	snprintf (dvd->oc->filename, sizeof(dvd->oc->filename), "%s", filename);
	...
	*/
	...
	{
		dvd->video_st = add_video_stream(dvd->oc, fmt->video_codec, kbit_rate);
		if (!dvd->video_st)
		{
			cleanup(dvd);
			return NULL;
	...
	{
		dvd->audio_st = add_audio_stream(dvd->oc, fmt->audio_codec);
		if (!dvd->audio_st)
		{
			cleanup(dvd);
			return NULL;
	...
	*/
	if (av_set_parameters(dvd->oc, NULL) < 0)
	{
	...
	*/
	...
	{
        if (url_fopen(&dvd->oc->pb, filename, URL_WRONLY) < 0)
		{
			cleanup(dvd);
	...
	initialise_tc_mask (dvd);	
	/*
	* write stream header if any 
	*/
	av_write_header(dvd->oc);

	return (dvd_encoder_t *)dvd;
}

static int write_audio_frame(internal_dvd_encoder_t *dvd, int16_t *p_audio)
{
	AVFormatContext *oc = dvd->oc;
	AVStream *st = dvd->audio_st;
	AVCodecContext *c;
...
static void burn_mask_yuv420(internal_dvd_encoder_t *dvd)
{
	int i,j;
...
static int write_video_frame(internal_dvd_encoder_t *dvd, uint8_t *p_video, int32_t frame_num)
{
	AVFormatContext *oc = dvd->oc;
	AVStream *st = dvd->video_st;
    int out_size, ret;
...
{
	internal_dvd_encoder_t *dvd = (internal_dvd_encoder_t *)in_dvd;
	if (!dvd)
		return -1;
