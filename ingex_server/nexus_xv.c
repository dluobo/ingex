...
extern int main(int argc, char *argv[])
{
	...
	for (n = 1; n < argc; n++)
	{
		...
		{
			if (sscanf(argv[n+1], "%d", &cardnum) != 1 ||
				cardnum > 4 || cardnum < 0)
			{
	...
	char					title_string[256] = {0};
	    ...
	    xvport = XVideoGetPort( display, X11_FOURCC('U','Y','V','Y'), -1);
	    if ( xvport < 0 )
	    {
	...
	while (1)
	{
		...
		if (verbose) {
			printf("\rcam%d lastframe=%d  tc=%10d  %s        ",
					cardnum, pc->lastframe,
					tc, framesToStr(tc, tcstr));
			fflush(stdout);
		}
