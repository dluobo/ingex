/*
 * $Id: burn_tc.c,v 1.1 2006/04/30 08:38:05 stuart_hc Exp $
 *
...
static int tc_width = 199, tc_height = 24;
static unsigned char tc_mask[tc_width * tc_height];

void setup_tc_mask(int frame_number)
{
...
static void burn_mask_yuv420(int x_offset, int y_offset, unsigned char *mask,
							int frame_width, int y_size, int u_size, unsigned char *frame)
{
	int i,j;
	...
	for (j = 0; j < m_height; j++)
	{
		for (i = 0; i < m_width; i++)
		{
			int y_idx =	y_offset * frame_width + x_offset +
						j * frame_width + i;
			int u_idx =	y_size + y_offset/2 * frame_width/2 + x_offset/2 +
						j/2 * frame_width/2 + i/2;
			int v_idx =	y_size + u_size + y_offset/2 * frame_width/2 + x_offset/2 +
						j/2 * frame_width/2 + i/2;
			unsigned char m1 = mask[j*m_width+ i];
			double		r1 = m1 / 255.0;

			// Y
			frame[y_idx] = r1 * (whiteY - frame[y_idx]) + frame[y_idx];

			if ((i % 2 == 0) && (j % 2 == 0))
			{
				// U V
				frame[u_idx] = r1 * (whiteU - frame[u_idx]) + frame[u_idx];
				frame[v_idx] = r1 * (whiteV - frame[v_idx]) + frame[v_idx];
			}
		}
	}
}
...
#if 0
	setup_tc_mask(tc_start_frame++);

	burn_mask_yuv420(tc_xoffset, tc_yoffset, tc_mask, width, y_size, u_size, frame);
#endif
