/*
 * $Id: MetadataSet.cpp,v 1.1 2009/02/02 05:14:33 stuart_hc Exp $
 *
 * 
 *
...
public:
    ...
    : _headerMetadata(headerMetadata), _value(0), _length(0), _haveNext(true), _referencedMetadataSet(0)
    {
    ...
    virtual bool next()
    {
        ...
        if (_haveNext)
        {
            ...
            if (_haveNext)
            {
                ...
                MXFPP_CHECK(mxf_get_strongref_s(_headerMetadata->getCHeaderMetadata(), &_setsIter, _value, &cSet));
...
mxfProductVersion MetadataSet::getProductVersionItem(const mxfKey* itemKey) const
{
    ...
    MXFPP_CHECK(mxf_get_product_version_item(_cMetadataSet, itemKey, &result));
    return result;
}
...
{
    ::MXFMetadataSet* cSet;
    MXFPP_CHECK(mxf_get_strongref_item(_cMetadataSet, itemKey, &cSet));
...
{
    ::MXFMetadataSet* cSet;
    MXFPP_CHECK(mxf_get_weakref_item(_cMetadataSet, itemKey, &cSet));
    return _headerMetadata->wrap(cSet);
}
